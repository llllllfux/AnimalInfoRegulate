...
import service.AMUtility;
import service.ArrayOfAnimal;
...
public class MainView {
    ...
    public void addMember() {
        ...
        if ('Y' == TSUtility.readConfirmSelection()) {
            ...
            Animal animalAdd = new Animal(AMUtility.speciesAdd(), AMUtility.genderAdd(), AMUtility.ageAdd());
            AnimalField animalField = new AnimalField();
            AMUtility.getAnimalFieldMap(animalField.getAnimalFieldAdd());
            animalAdd.setAnimalFiled(animalField);

            //判断要添加的数据列表中是否已经存在;
            int size = ArrayOfAnimal.duplicationRemove(animalAdd);
            if (size == 0) {
                //要添加的数据不存在
    ...
    public void deleteMember() {
        ...
        if (deleteNum == 0) {
            ...
            //判断此数据是否存在
            int exist = ArrayOfAnimal.isExist(ArrayOfAnimal.getAnimals(), deleteNum - 1);
            if (exist == -1) {
                //数据不存在
    ...
    public void modifyMember() {
        ...
        if ('Y' == TSUtility.readConfirmSelection()) {
            ...
            //判断要修改的数据是否存在
            int modifyNum = ArrayOfAnimal.isExist(ArrayOfAnimal.getAnimals(), keyboardNum - 1);
            if (modifyNum != -1) {
                //数据存在
                Animal animalModify = new Animal(AMUtility.speciesModify(), AMUtility.genderModify(), AMUtility.ageModify());
                AnimalField newAnimalField = new AnimalField();
                AMUtility.getAnimalFieldMap(newAnimalField.getAnimalFieldAdd());
    ...
    public void sortByAge() {
        ...
        //深拷贝一个集合
        ArrayList<Animal> copy = ArrayOfAnimal.copy(ArrayOfAnimal.getAnimals(), new ArrayList<Animal>());
    ...
    public void queryAnimal() {
        ...
        //判断此数据是否存在
        int exist = ArrayOfAnimal.isExist(ArrayOfAnimal.getAnimals(), animalNum - 1);
        if (exist == -1) {
            //数据不存在
